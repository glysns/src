...
import digytal.desktop.components.desktop.ss.tabela.SSModeloColuna;
import digytal.desktop.components.desktop.ss.tabela.SSModeloTabela;
import digytal.desktop.components.desktop.ss.tabela.SSTipoSelecao;
import digytal.desktop.components.desktop.ss.util.SSFormatador;
import digytal.desktop.components.desktop.ss.util.SSReflexao;
...
public class SSGrade extends JTable {
	...
	private boolean porAtributo;
	public void setPorAtributo(boolean porAtributo) {
		this.porAtributo = porAtributo;
	}
		...
		try {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				for(int y=0;y<fields.length;y++) {
					SSReflexao.definirValorCampo(item, fields[y], getModel().getValueAt(x, editaveis[y]),porAtributo);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void carregraGrade() {
		...
		for (Object registro : (List) value) {
			...
			for (int i = 0; i < modeloColuna.getColumnCount(); i++) {
				...
				try {
					valor = SSReflexao.buscarValorCampoRecursivo(registro, nomeCampo,porAtributo);
					...
					if (formato == null && valor != null) {
						if (valor instanceof Boolean)
							...
							LocalDate dt = (LocalDate) valor;
							valor = dt.format(DateTimeFormatter.ofPattern("dd/MM/yy"));
						}
						else if (valor instanceof LocalDateTime) {
							LocalDateTime dt = (LocalDateTime) valor;
							valor = dt.format(DateTimeFormatter.ofPattern("dd/MM/yy HH:mm"));
						}
					}

					//FIXME: Estudar formatacao campos
	...
	public int definirLarguraColunas(int... larguras) {
		int col = 0;
		int size = 0;
