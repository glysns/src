...
public class FrmCatagoriaConsulta extends FormularioConsulta {
	
	@Autowired
	private CategoriaService service;
	private SSCampoTexto cNome = new SSCampoTexto();
	public FrmCatagoriaConsulta() {
		setTitulo("Categorias");
		setDescricao("Listagem das Categorias dos Produtos");
		...
		bIncluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				incluir(Context.getBean(FrmCategoriaCadastro.class));
			}
		});
		
		bAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				alterar(Context.getBean(FrmCategoriaCadastro.class));
			}
		});
	...
	public void filtrar() {
		try {
			...
			if (nome.isEmpty()) {
				SSMensagem.avisa("Informe um nome da Categoria para a pesquisa");
				return;
			}
			List<CategoriaResponse> registros = service.consultar(nome);
